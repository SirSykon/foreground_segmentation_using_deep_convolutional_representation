...
def autoencode_video(video_images_paths, num_training_images, autoencoder, video_name, category):
    for index, video_image_path in enumerate(video_images_paths):
        ...
        segmented_image_path = os.path.join(configuration.MODEL_FOLDER_PATH,
                                                    "aut_test", 
                                                    category, 
                                                    video_name, 
...
autoencoder = Autoencoder.Convolutional_Autoencoder(configuration.MODEL_FOLDER_PATH, load = True)
...
for (category, video_name) in datasets_utils.get_change_detection_categories_and_videos_list(filter_value = "canoe"):
    ...
    autoencode_folder = os.path.join(configuration.MODEL_FOLDER_PATH, 
                                        "aut_test",
                                        category, 
                                        video_name)
