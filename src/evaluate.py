...
all_data_df = pd.DataFrame(columns = ["method", "noise", "category", "video", "TP", "TN", "FP", "FN", "precision", "recall", "TNR", "FNR", "f-measure"])         # We initialize the all data dataframe.

for method, method_output_path in zip(configuration.EVALUATE_METHODS_LIST, configuration.ALL_SEGMENTATION_OUTPUT_FOLDER):
    for (noise, category, video_name) in datasets_utils.get_change_detection_noises_categories_and_videos_list(filter_value = None):              # For each category and video.
        print(method)
        print(method_output_path)
        print(noise)
        print(category)
        print(video_name)
        if category in configuration.CATEGORIES_TO_TEST and noise != "jpeg_compression_10" and noise != "jpeg_compression_1":
            ...
            segmentation_folder = os.path.join(method_output_path,
                                                noise, 
                                                category, 
            ...
            video_row_serie = pd.Series([method, noise, category, video_name, TP_video_sum, TN_video_sum, FP_video_sum, FN_video_sum, precision_video_sum, recall_video_sum, TNR_video_sum, FNR_video_sum, f_measure_video_sum], index = all_data_df.columns)                                                      # We initialize the serie.
            video_row_serie.name = method+"_"+noise+"_"+category+"_"+video_name                                            # We set a name to the serie.
    ...
    evaluation_folder = configuration.EVALUATION_OUTPUT_FOLDER
